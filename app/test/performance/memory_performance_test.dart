...
void main() {
  BlurAppTestFramework.testGroup('Memory & Performance Tests', () {
    ...
    group('[PERFORMANCE] Image Processing Memory Tests', () {
      BlurAppTestFramework.testCase('small image processing stays within memory limits', () {
        ...
        final gaussianResult = BlurPipeline.applyBlur(imageBytes, BlurType.gaussian, 5);
        final pixelateResult = BlurPipeline.applyBlur(imageBytes, BlurType.pixelate, 8);
        final mosaicResult = BlurPipeline.applyBlur(imageBytes, BlurType.mosaic, 10);
      ...
      BlurAppTestFramework.testCase('medium image processing performance', () {
        final imageBytes = createTestImage(width: 800, height: 600, complex: true);
      ...
      BlurAppTestFramework.testCase('large image memory constraint validation', () {
    ...
    group('[PERFORMANCE] AutoDetectService Memory Tests', () {
      BlurAppTestFramework.asyncTest('face detection suggestions use minimal memory', () async {
      ...
      BlurAppTestFramework.asyncTest('multiple service instances cleanup properly', () async {
    ...
    group('[PERFORMANCE] Image Save/Load Operations', () {
      BlurAppTestFramework.asyncTest('image save operations handle large files efficiently', () async {
        final stopwatch = Stopwatch()..start();
        final largeImageBytes = createTestImage(width: 1200, height: 900, complex: true);

        final String? tempPath = await ImageSaverService.saveImage(largeImageBytes);
        final String? permanentPath = await ImageSaverService.saveImagePermanent(largeImageBytes);
      ...
      BlurAppTestFramework.asyncTest('cache clearing handles large directories', () async {
      ...
      BlurAppTestFramework.asyncTest('concurrent image processing maintains stability', () async {
        ...
        for (int i = 0; i < 5; i++) {
          final future = Future(() {
            return BlurPipeline.applyBlur(imageBytes, BlurType.gaussian, 5 + i);
          });
          futures.add(future);
        }
    ...
    group('[PERFORMANCE] Edge Cases and Limits', () {
      BlurAppTestFramework.testCase('extremely small images are handled gracefully', () {
        final tinyImageBytes = createTestImage(width: 1, height: 1);

        final result = BlurPipeline.applyBlur(tinyImageBytes, BlurType.gaussian, 1);
        expect(result.isNotEmpty, isTrue);
      }, level: TestLevel.misc);

      BlurAppTestFramework.testCase('invalid image data fails gracefully without memory leaks', () {
        final invalidBytes = Uint8List.fromList([1, 2, 3, 4]); // Not an image

        final result = BlurPipeline.applyBlur(invalidBytes, BlurType.gaussian, 5);

        // Should return original bytes on failure
        expect(result, equals(invalidBytes));
      }, level: TestLevel.core);

      BlurAppTestFramework.testCase('extreme blur strength values are handled safely', () {
        final imageBytes = createTestImage(width: 200, height: 200);

        // Test extreme values
        final lowResult = BlurPipeline.applyBlur(imageBytes, BlurType.pixelate, -5);
        final highResult = BlurPipeline.applyBlur(imageBytes, BlurType.pixelate, 1000);
