...
void main() {
  ...
  group('Phase 2: OpenCV Blur Engine Tests', () {
        ...
        switch (methodCall.method) {
          ...
          case 'applyAdvancedBlur':
            ...
            if (sigma > 0 && imageBytes.isNotEmpty && blurType >= 0) {
              ...
              final factor = blurType == 1 ? 0.7 : 0.8; // Different effect for box blur
              final result = Uint8List.fromList(imageBytes.map((b) => (b * factor).round().clamp(0, 255)).toList());
              return result;
            }
            return Uint8List(0);
          ...
          case 'applySelectiveBlur':
            ...
            if (imageBytes.isNotEmpty && maskBytes.isNotEmpty) {
              ...
              final result = Uint8List.fromList(imageBytes.map((b) => (b * blurFactor).round().clamp(0, 255)).toList());
              return result;
            }
            return Uint8List(0);
    ...
    test('should apply selective blur with mask', () async {
      final testImage = Uint8List.fromList(List.generate(1000, (i) => i % 256));
      final testMask = Uint8List.fromList(List.generate(250, (i) => i % 2 == 0 ? 255 : 0));
    ...
    test('should handle zero sigma gracefully', () async {
      ...
      final result = await NativeBlurBindings.applyAdvancedBlur(testImage, sigma: 0.0, blurType: 0);
    ...
    test('should handle large images efficiently', () async {
      ...
      final largeImage = Uint8List.fromList(List.generate(1024 * 1024 * 4, (i) => i % 256));

      final result = await NativeBlurBindings.applyAdvancedBlur(largeImage, sigma: 1.5, blurType: 0);
