...
void main() {
  BlurAppTestFramework.testGroup(
    'Enhanced Memory Management Tests',
    () {
      ...
      group('[CRITICAL] Memory Safety Features', () {
        ...
        BlurAppTestFramework.testCase(
          'preview mode reduces memory usage',
          () {
            final largeImageBytes =
                createLargeTestImage(width: 2000, height: 1500);

            // Test normal processing
            final normalResult =
                BlurPipeline.applyBlur(largeImageBytes, BlurType.pixelate, 8);
            final normalImage = img.decodeImage(normalResult)!;

            // Test preview processing
            final previewResult = BlurPipeline.applyBlur(
                largeImageBytes, BlurType.pixelate, 8,
                isPreview: true);
        ...
        BlurAppTestFramework.testCase(
          ...
          () {
            ...
            final negativeResult =
                BlurPipeline.applyBlur(imageBytes, BlurType.gaussian, -100);
            expect(negativeResult.isNotEmpty, isTrue);

            // Test extreme positive value
            final extremeResult =
                BlurPipeline.applyBlur(imageBytes, BlurType.gaussian, 10000);
      ...
      group('[CRITICAL] Performance Optimizations', () {
        ...
        BlurAppTestFramework.testCase(
          ...
          () {
            ...
            expect(previewTime.milliseconds,
        ...
        BlurAppTestFramework.testCase(
          ...
          () {
            ...
            for (final size in sizes) {
              final imageBytes =
                  createLargeTestImage(width: size[0], height: size[1]);
      ...
      group('[CORE] Edge Case Handling', () {
        BlurAppTestFramework.testCase(
          'ultra-high resolution image handling',
          () {
            // Test with unrealistic resolution to verify safety limits
            final ultraHighResBytes =
                createLargeTestImage(width: 8000, height: 6000);

            final result = BlurPipeline.applyBlur(
                ultraHighResBytes, BlurType.pixelate, 20);
            expect(result.isNotEmpty, isTrue);
        ...
        BlurAppTestFramework.testCase(
          'concurrent large image processing',
          () {
            ...
            final timeMeasurement =
              ...
              for (int i = 0; i < 3; i++) {
                final result = BlurPipeline.applyBlur(
                    imageBytes, BlurType.gaussian, 5 + i);
                results.add(result);
              }
            });
        ...
        BlurAppTestFramework.testCase(
          ...
          () {
            ...
            for (int i = 0; i < 5; i++) {
              final result =
                  BlurPipeline.applyBlur(imageBytes, BlurType.mosaic, 10);
              expect(result.isNotEmpty, isTrue);
            }
