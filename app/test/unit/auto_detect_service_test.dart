...
void main() {
  BlurAppTestFramework.testGroup(
    'AutoDetectService Manual Fallback Tests',
    () {
      ...
      BlurAppTestFramework.asyncTest(
        'can create face detection service',
        () async {
          ...
          expect(service.isAutomaticDetectionAvailable,
              isFalse); // Manual fallback
          service.close();
        },
      ...
      BlurAppTestFramework.asyncTest(
        'can create segmentation service',
        () async {
          final service = await AutoDetectService.create(
      ...
      BlurAppTestFramework.asyncTest(
        'throws error for unknown model',
        () async {
          expect(
            () async => await AutoDetectService.create(
                modelPath: 'unknown_model.tflite'),
      ...
      BlurAppTestFramework.asyncTest(
        'face detection returns suggested regions',
        () async {
          final service = await AutoDetectService.create(
              modelPath: 'assets/models/face_detection_short_range.tflite');
      ...
      BlurAppTestFramework.asyncTest(
        'segmentation returns full image region',
        () async {
          final service = await AutoDetectService.create(
              modelPath: 'assets/models/selfie_segmentation.tflite');
      ...
      BlurAppTestFramework.asyncTest(
        'segmentation mask generation works',
        () async {
          final service = await AutoDetectService.create(
              modelPath: 'assets/models/selfie_segmentation.tflite');
      ...
      BlurAppTestFramework.asyncTest(
        'face detection service returns null for segmentation',
        () async {
          final service = await AutoDetectService.create(
              modelPath: 'assets/models/face_detection_short_range.tflite');
          ...
          expect(
              maskBytes, isNull); // Face detection doesn't provide segmentation
      ...
      BlurAppTestFramework.asyncTest(
        'handles invalid image data gracefully',
        () async {
          final service = await AutoDetectService.create(
              modelPath: 'assets/models/face_detection_short_range.tflite');
      ...
      BlurAppTestFramework.asyncTest(
        'landscape images get multiple face suggestions',
        () async {
          final service = await AutoDetectService.create(
              modelPath: 'assets/models/face_detection_short_range.tflite');
          ...
          expect(
              result.length, greaterThan(1)); // Should suggest multiple regions
