...
void main() {
  BlurAppTestFramework.testGroup('BlurPipeline Core Algorithm Tests', () {
    BlurAppTestFramework.testCase('gaussian blur produces valid output for all strengths', () {
      ...
      for (int strength = 1; strength <= 30; strength++) {
        ...
        final result = BlurPipeline.applyBlur(testImageBytes, BlurType.gaussian, strength);

        BlurAppAssertions.assertValidImageResult(result);
      }
    }, level: TestLevel.core);

    BlurAppTestFramework.testCase('pixelate effect maintains image integrity', () {
      final testImageBytes = TestHelpers.createTestImageBytes();

      final result = BlurPipeline.applyBlur(testImageBytes, BlurType.pixelate, 10);
    ...
    BlurAppTestFramework.testCase('handles edge case blur strength values', () {
      ...
      final result1 = BlurPipeline.applyBlur(testImageBytes, BlurType.gaussian, 0);
      expect(result1, isNotNull); // Current implementation returns data

      final result2 = BlurPipeline.applyBlur(testImageBytes, BlurType.gaussian, -1);
