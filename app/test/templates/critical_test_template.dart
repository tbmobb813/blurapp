...
void main() {
  BlurAppTestFramework.testGroup('Critical User Flow Tests', () {
    ...
    BlurAppTestFramework.widgetTest(
      ...
      (tester) async {
        ...
        await tester.pumpWidget(
          BlurAppTestFramework.createTestApp(const _MockEditorScreen()),
        );
        ...
        await TestHelpers.tapAndSettle(
          tester,
          find.byKey(const Key('blur_button')),
        );
    ...
    BlurAppTestFramework.integrationTest('complete app flow works end-to-end', (
      tester,
    ) async {
      await tester.pumpWidget(
        BlurAppTestFramework.createTestApp(const _MockBlurApp()),
      );
...
class _MockBlurApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ElevatedButton(onPressed: () {}, child: const Text('Start Editing')),
      ],
    );
  }
}
