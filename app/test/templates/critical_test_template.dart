  ...
  BlurAppTestFramework.testGroup(
    ...
    () {
      ...
      BlurAppTestFramework.widgetTest(
        'user can complete full blur workflow from image selection to export',
        (tester) async {
          // Build the main app
          await tester.pumpWidget(BlurAppTestFramework.createTestApp(
            const _MockEditorScreen(),
          ));

          // Step 1: User selects image
          await TestHelpers.tapAndSettle(
              tester, find.byKey(const Key('gallery_button')));

          BlurAppAssertions.assertUIStateTransition(
              tester, 'Select Image', 'Image Loaded');

          // Step 2: User applies blur
          await TestHelpers.tapAndSettle(
              tester, find.byKey(const Key('blur_button')));

          BlurAppAssertions.assertUIStateTransition(
              tester, 'Original', 'Blurred');

          // Step 3: User exports image
          await TestHelpers.tapAndSettle(
              tester, find.byKey(const Key('export_button')));
      ...
      BlurAppTestFramework.widgetTest(
        'app gracefully handles image processing errors',
        (tester) async {
          await tester.pumpWidget(BlurAppTestFramework.createTestApp(
            const _MockEditorScreen(),
          ));
      ...
      BlurAppTestFramework.asyncTest(
        'user changes are preserved across app lifecycle',
        () async {
      ...
      BlurAppTestFramework.integrationTest(
        'complete app flow works end-to-end',
        (tester) async {
          await tester.pumpWidget(BlurAppTestFramework.createTestApp(
            const _MockBlurApp(),
          ));
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        ElevatedButton(
          key: const Key('error_trigger_button'),
          onPressed: () {},
